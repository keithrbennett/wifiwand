...
RSpec.describe WifiWand::Platforms::Mac::Helper::SetupCli do
  before do
    allow(WifiWand::Platforms::Mac::Helper::Bundle).to receive(:helper_install_dir_count).and_return(0)
  end
  ...
  def build_result(installed: true, valid: true, authorized: false, message: 'Not authorized')
    WifiWand::Platforms::Mac::Helper::Setup::Result.new(
      installed:          installed,
      valid:              valid,
  ...
  def build_support_status(version)
    WifiWand::Platforms::Mac::Helper::Setup::SupportStatus.new(
      macos_version:  version,
      parsed_version: WifiWand::Platforms::Mac::Helper::Bundle.parse_macos_version(version)
    )
  end
  ...
  def stub_setup(setup, initial_status:, post_status: nil)
    ...
    allow(WifiWand::Platforms::Mac::Helper::Bundle)
  ...
  describe 'when macOS is older than the helper minimum' do
    let(:setup) { instance_double(WifiWand::Platforms::Mac::Helper::Setup) }
  ...
  describe 'helper version directory notice' do
    ...
    before do
      allow(WifiWand::Platforms::Mac::Helper::Bundle).to receive_messages(
    ...
    it 'prints an advisory notice after installing when helper version directories exceed the threshold' do
      ...
      expect(out_stream.string).to include(WifiWand::Platforms::Mac::Helper::Bundle::INSTALL_PARENT)
    ...
    it 'does not print the notice when the directory count is at the threshold' do
      allow(WifiWand::Platforms::Mac::Helper::Bundle)
        ...
        .and_return(WifiWand::Platforms::Mac::Helper::Bundle::VERSION_DIRECTORY_NOTICE_THRESHOLD)
      allow(setup).to receive(:check_status).and_return(missing_status, complete_status)
