...
require_relative '../../../../lib/wifi_wand/models/mac_os/network_identity_reader'

module WifiWand
  describe MacOsNetworkIdentityReader do
    subject(:reader) do
      described_class.new(
        helper_client_proc:            -> { helper_client },
        command_runner:                command_runner,
        airport_data_proc:             ->(timeout_in_secs: nil) { airport_data_proc.call(timeout_in_secs) },
    ...
    describe '#connected_network_name_raw' do
      ...
      it 'uses networksetup before airport data' do
    ...
    describe '#connected?' do
      it 'ignores helper placeholder names and falls back to association without SSID' do
        ...
        allow(airport_data_proc).to receive(:call).and_return(airport_payload(current_network_name: :missing))
