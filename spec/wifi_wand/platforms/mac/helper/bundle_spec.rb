...
RSpec.describe WifiWand::Platforms::Mac::Helper::Bundle do
  ...
  describe WifiWand::Platforms::Mac::Helper::Client do
    ...
    describe WifiWand::Platforms::Mac::Helper::Bundle::HelperQueryResult do
    ...
    describe '#execute_helper_command' do
      ...
      it 'passes the command-specific timeout to the helper runner' do
        allow(helper_bundle).to receive(:run_bounded_helper_command).and_return(
    ...
    describe '#ensure_helper_installed' do
      ...
      before do
        allow(helper_bundle).to receive(:installed_executable_path).and_return(helper_path)
      end
      ...
      context 'when the helper executable is present and valid' do
        it 'returns immediately without reinstalling' do
          ...
          expect(helper_bundle).to receive(:helper_installed_and_valid?).and_return(true)
        ...
        it 'caches successful validation for subsequent calls' do
          ...
          expect(helper_bundle).to receive(:helper_installed_and_valid?).and_return(true).once
    ...
    describe 'helper-backed calls after install failure' do
      ...
      before do
        ...
        allow(helper_bundle).to receive_messages(installed_executable_path: '/tmp/helper',
          helper_installed_and_valid?: false)
        allow(helper_bundle).to receive(:ensure_helper_installed).and_raise(StandardError,
          'boom')
      end
      ...
      it 'does not retry helper installation on a later helper-backed call in the same process' do
        ...
        expect(helper_bundle).to have_received(:ensure_helper_installed).once
    ...
    describe '.sanitize_macos_version' do
      ...
      it 'removes prerelease suffixes like beta tags' do
        expect(helper_bundle.sanitize_macos_version('15.6.1-beta2')).to eq('15.6.1')
      end

      it 'returns nil when the version does not include numeric components' do
        expect(helper_bundle.sanitize_macos_version('unknown')).to be_nil
      end
    end

    describe '.helper_supported_on_macos_version?' do
      it 'returns false below the minimum helper version' do
        expect(helper_bundle.helper_supported_on_macos_version?('13.6.1')).to be(false)
      end

      it 'returns true at the minimum helper version' do
        expect(helper_bundle.helper_supported_on_macos_version?('14.0')).to be(true)
      end

      it 'returns true above the minimum helper version with build metadata' do
        expect(helper_bundle.helper_supported_on_macos_version?('15.6 (24G84)')).to be(true)
      end

      it 'returns false when the version is missing' do
        expect(helper_bundle.helper_supported_on_macos_version?(nil)).to be(false)
      end

      it 'returns false when the version is malformed' do
        expect(helper_bundle.helper_supported_on_macos_version?('developer seed')).to be(false)
      end
    end
    ...
    describe '.helper_support_status_for_macos_version' do
      it 'parses the version once for callers that need status and diagnostics' do
        expect(helper_bundle).to receive(:parse_macos_version).with('developer seed').once
          .and_call_original

        status = helper_bundle.helper_support_status_for_macos_version('developer seed')
    ...
    describe '.detect_macos_version' do
      ...
      it 'returns the stripped sw_vers product version' do
        ...
        expect(helper_bundle.detect_macos_version).to eq('15.6')
      end
      ...
      it 'returns nil when sw_vers exits successfully with empty output' do
        ...
        expect(helper_bundle.detect_macos_version).to be_nil
      end
