    ...
    describe 'Unit tests for each error class' do
      ...
      error_test_cases = [
        ...
        [InvalidIPAddressError,         ['999.999.999.999'],
          "Invalid IP address(es): 999.999.999.999. To remove all DNS nameservers, use 'clear'."],
        [InvalidIPAddressError,         [['1.2.3.4.5', 'abc']],
          "Invalid IP address(es): 1.2.3.4.5, abc. To remove all DNS nameservers, use 'clear'."],
    ...
    describe 'Method integration tests' do
      ...
      error_raising_test_cases = [
        ...
        },
        {
          ...
          before: -> {
            allow(model).to receive_messages(
              run_command: command_result(stdout: ''), wifi_on?: false
            )
        ...
        },
        {
          ...
          before: -> {
            allow(model).to receive_messages(
              run_command: command_result(stdout: ''), wifi_on?: true
            )
      ...
      describe 'macOS specific errors' do
        ...
        keychain_error_test_cases.each do |test_case|
          it "raises #{test_case[:error]} for exit code #{test_case[:exit_code]}" do
            # This mock intercepts calls to `run_command` to simulate `security` command failures.
            allow(mac_model).to receive(:run_command) do |*args, **_kwargs|
              command = args.first
