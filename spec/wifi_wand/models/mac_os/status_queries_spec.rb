...
require_relative '../../../../lib/wifi_wand/models/mac_os/status_queries'

module WifiWand
  describe MacOsStatusQueries do
    ...
    it 'does not predefine the macOS OS detector when required before os/mac_os' do
      code = <<~RUBY
        require 'wifi_wand/models/mac_os/status_queries'
        require 'wifi_wand/os/mac_os'

        puts WifiWand::MacOs.superclass
        RUBY
      stdout, stderr, status = Open3.capture3(RbConfig.ruby, '-Ilib', '-e', code)

      expect(status).to be_success, stderr
      expect(stdout).to eq("WifiWand::BaseOs\n")
    end
