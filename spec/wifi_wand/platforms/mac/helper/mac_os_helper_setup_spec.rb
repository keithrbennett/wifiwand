...
require 'spec_helper'
require 'stringio'
require 'wifi_wand/platforms/mac/helper/mac_os_helper_setup'
...
RSpec.describe WifiWand::Platforms::Mac::Helper::Setup do
  ...
  # Result value object
  # ---------------------------------------------------------------------------
  describe WifiWand::Platforms::Mac::Helper::Setup::Result do
  ...
  # ---------------------------------------------------------------------------
  # Platforms::Mac::Helper::Setup#helper_support_status
  # ---------------------------------------------------------------------------
  describe '#helper_support_status' do
    ...
    context 'when no version proc is injected' do
      ...
      it 'uses the centralized macOS version detector' do
        allow(helper_bundle).to receive(:detect_macos_version).and_return('14.0')
  ...
  # ---------------------------------------------------------------------------
  # Platforms::Mac::Helper::Setup#check_status
  # ---------------------------------------------------------------------------
  describe '#check_status' do
    ...
    context 'when the helper is not installed' do
      before do
        ...
        allow(helper_bundle).to receive(:helper_installed_and_valid?).and_return(false)
      end
      ...
      it 'does not attempt to invoke the helper executable' do
        expect(helper_bundle).not_to receive(:run_bounded_helper_command)
        setup.check_status
      end
    end
  ...
  # ---------------------------------------------------------------------------
  # Platforms::Mac::Helper::Setup#install_helper
  # ---------------------------------------------------------------------------
  describe '#install_helper' do
    it 'delegates to Platforms::Mac::Helper::Bundle.ensure_helper_installed with the output stream' do
      expect(helper_bundle)
  ...
  # ---------------------------------------------------------------------------
  # Platforms::Mac::Helper::Setup#reinstall_helper
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # Platforms::Mac::Helper::Setup#remove_helper
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # Platforms::Mac::Helper::Setup#open_location_settings
  # ---------------------------------------------------------------------------
