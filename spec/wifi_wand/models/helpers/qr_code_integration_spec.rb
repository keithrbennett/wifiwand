...
describe 'QR Code Integration Tests', :requires_external_tools do
  ...
  def real_qrencode_result(cmd)
    stdout, stderr, status = Open3.capture3(*cmd)
    command_result(
      stdout:     qrencode_binary_output?(cmd) ? stdout.b : stdout,
      stderr:     stderr,
  ...
  def qrencode_binary_output?(cmd)
    cmd[2] == 'PNG'
  end
  ...
  describe 'Real QR Code Generation and Verification' do
    ...
    test_cases.each do |test_case|
      it "creates valid QR code for #{test_case[:name]}" do
        ...
        # Capture real qrencode stdout so production writes the generated bytes.
        allow(test_model).to receive(:run_command) do |cmd|
          if cmd.is_a?(Array) && cmd[0] == 'qrencode'
            real_qrencode_result(cmd)
          else
