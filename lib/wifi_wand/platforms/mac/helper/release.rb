    ...
    module Mac
      module Helper
        module Release
          ...
          APPLE_TEAM_ID_ENV = Bundle::APPLE_TEAM_ID_ENV
          CODESIGN_IDENTITY_ENV = Bundle::CODESIGN_IDENTITY_ENV
          ...
          module Operations
            def self.require_macos!(task_name)
              return if RbConfig::CONFIG['host_os'] =~ /darwin/i

              abort "#{task_name} is only supported on macOS hosts."
            end

            def self.helper_executable_path
              helper = Bundle
              File.join(helper.source_bundle_path, 'Contents', 'MacOS', Bundle::EXECUTABLE_NAME)
            end
            ...
            def self.verify_universal_binary?(binary_path)
              ...
              unless has_arm64 && has_intel_architecture
                ...
                missing_platform = missing.include?('arm64') ? 'Apple Silicon' : 'Intel'
                puts "  This binary will not work on #{missing_platform} Macs."
                return false
              end
            ...
            def self.verify_credentials(profile_name, team_id,
          ...
          module_function def build_signed_helper
            ...
            destination = File.join(helper.source_bundle_path, 'Contents', 'MacOS', Bundle::EXECUTABLE_NAME)
          ...
          module_function def notarization_status(submission_id)
            ...
            creds = fetch_notary_credentials!(
              command_hint: 'bin/mac-helper-release info --submission-id <uuid>'
            )
          ...
          module_function def notarization_log(submission_id)
            ...
            creds = fetch_notary_credentials!(
              command_hint: 'bin/mac-helper-release log --submission-id <uuid>'
            )
          ...
          module_function def cancel_notarization(submission_id)
            ...
            creds = fetch_notary_credentials!(
              command_hint: 'bin/mac-helper-release cancel --submission-id <uuid>'
            )
          ...
          module_function def configured_codesign_identity
            Bundle.configured_codesign_identity
          end

          module_function def configured_team_id
            Bundle.configured_team_id
          end
        end
      end
    end
