...
Use the cov-loupe MCP server to analyze the test coverage data generated by SimpleCov for this Ruby project.
Prefer MCP tools such as `project_coverage`, `file_coverage_summary`, and `file_uncovered_lines` over reading
resultsets directly; only fall back to the `cov-loupe` CLI if the MCP server is unavailable. Generate a
detailed coverage analysis report and output it to a markdown file named `test_coverage_analysis.md`.
For whole-project analysis, call `project_coverage` with the intended runtime file set so files missing from
the SimpleCov resultset are reported explicitly instead of being ignored.
...
Organize by:
- **Complexity to Test**: Simple/Moderate/Complex (considering dependencies, I/O operations, external
  services, etc.)
...
Include specific file paths, line numbers, and code examples where relevant. Focus on actionable insights
rather than just reporting numbers. Provide concrete recommendations for improving the test suite architecture
