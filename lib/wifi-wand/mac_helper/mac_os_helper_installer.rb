  ...
  module MacOsHelperInstaller
    ...
    module_function def install_helper_bundle(out_stream: $stdout)
      with_install_lock do
        ...
        Dir.mktmpdir(
          "#{MacOsHelperBundle::BUNDLE_NAME}.tmp-",
          MacOsHelperBundle.versioned_install_dir
        ) do |temp_dir|
          staged_bundle_path = File.join(temp_dir, MacOsHelperBundle::BUNDLE_NAME)
    ...
    module_function def install_lock_path
      File.join(MacOsHelperBundle.versioned_install_dir, '.install.lock')
    end
    ...
    module_function def installed_bundle_current?
      ...
      if manifest
        manifest['helper_version'] == MacOsHelperBundle.helper_version &&
          manifest['bundle_fingerprint'] ==
            MacOsHelperBundle.bundle_fingerprint(MacOsHelperBundle.installed_bundle_path) &&
          manifest['bundle_fingerprint'] ==
            MacOsHelperBundle.bundle_fingerprint(MacOsHelperBundle.source_bundle_path)
      else
        false
      end
    end
    ...
    module_function def helper_exited_within_grace_period?(wait_thr)
      !!wait_thr.join(MacOsHelperBundle::HELPER_TERMINATION_WAIT_SECONDS)
    end

    module_function def with_install_lock
      FileUtils.mkdir_p(MacOsHelperBundle.versioned_install_dir)
    ...
    module_function def stage_helper_bundle(staged_bundle_path)
      FileUtils.cp_r(MacOsHelperBundle.source_bundle_path, staged_bundle_path)
    end
    ...
    module_function def publish_release_symlink(release_bundle_path, publish_token)
      ...
      if File.symlink?(MacOsHelperBundle.installed_bundle_path)
      ...
      if File.exist?(MacOsHelperBundle.installed_bundle_path)
    ...
    module_function def bundle_release_path(publish_token) =
      File.join(MacOsHelperBundle.versioned_install_dir, ".#{MacOsHelperBundle::BUNDLE_NAME}.release-#{publish_token}")

    module_function def staged_bundle_symlink_path(publish_token)
      "#{MacOsHelperBundle.installed_bundle_path}.link-#{publish_token}"
    end

    module_function def legacy_executable_symlink_path(publish_token)
      "#{MacOsHelperBundle.installed_executable_path}.link-#{publish_token}"
    end

    module_function def legacy_info_plist_path
      File.join(MacOsHelperBundle.installed_bundle_path, 'Contents', 'Info.plist')
    end

    module_function def legacy_code_resources_path =
      File.join(MacOsHelperBundle.installed_bundle_path, 'Contents', '_CodeSignature', 'CodeResources')

    module_function def resolved_installed_bundle_target
      return unless File.symlink?(MacOsHelperBundle.installed_bundle_path)

      File.expand_path(
        File.readlink(MacOsHelperBundle.installed_bundle_path),
        MacOsHelperBundle.versioned_install_dir
      )
    end
    ...
    module_function def cleanup_previous_release(previous_release_path)
      ...
      prefix = "#{MacOsHelperBundle.versioned_install_dir}/.#{MacOsHelperBundle::BUNDLE_NAME}.release-"
    ...
    module_function def switch_legacy_bundle_executable(release_bundle_path, publish_token)
      ...
      release_executable_path = File.join(release_bundle_path, 'Contents', 'MacOS',
        MacOsHelperBundle::EXECUTABLE_NAME)
    ...
    module_function def resolved_legacy_release_target
      return unless File.symlink?(MacOsHelperBundle.installed_executable_path)

      File.expand_path(
        File.readlink(MacOsHelperBundle.installed_executable_path),
        MacOsHelperBundle.versioned_install_dir
      )
    ...
    module_function def write_manifest
      FileUtils.mkdir_p(MacOsHelperBundle.versioned_install_dir)
      File.write(
        File.join(MacOsHelperBundle.versioned_install_dir, 'VERSION'),
        MacOsHelperBundle.helper_version
      )
      File.write(install_manifest_path, JSON.pretty_generate(
        {
          'helper_version'     => MacOsHelperBundle.helper_version,
          'bundle_fingerprint' => MacOsHelperBundle.bundle_fingerprint(MacOsHelperBundle.source_bundle_path),
        }
      ))
    end
