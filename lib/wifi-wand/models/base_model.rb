...
module WifiWand
  class BaseModel
    ...
    def connect(network_name, password = nil, skip_saved_password_lookup: false)
      debug_method_entry(__method__, binding, [:network_name, :password])
    ...
    def till(target_status, timeout_in_secs: nil, wait_interval_in_secs: nil,
      ...
      debug_method_entry(__method__, binding,
        [:target_status, :timeout_in_secs, :wait_interval_in_secs])
    ...
    def try_os_command_until(command, stop_condition, max_tries = 100)
      debug_method_entry(__method__, binding, [:command, :stop_condition, :max_tries])
    ...
    def restore_network_state(state, fail_silently: false)
      debug_method_entry(__method__, binding, [:state, :fail_silently])
