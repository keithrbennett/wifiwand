    ...
    context 'core functionality tests (non-disruptive)' do
      ...
      describe '#_connect early return and branches' do
        ...
        it 'modifies existing profile when password changed and security is known' do
          ...
          expect(subject).to receive(:run_os_command).with(['nmcli', 'connection', 'modify', 'SSID1',
                                                            '802-11-wireless-security.psk', 'newpass']).and_return(command_result(stdout: ''))
          expect(subject).to receive(:run_os_command).with(['nmcli', 'connection', 'up',
        ...
        it 'falls back to direct connect when security cannot be determined' do
          ...
          expect(subject).to receive(:run_os_command).with(['nmcli', 'dev', 'wifi', 'connect',
                                                            'SSID2', 'password', 'newpass']).and_return(command_result(stdout: ''))
        ...
        it 'raises NetworkNotFoundError when network not in range' do
          ...
          expect do
            subject._connect('SSID4', 'pw')
          end.to raise_error(WifiWand::NetworkNotFoundError, /SSID4/)
        end
        ...
        it 'raises NetworkAuthenticationError when password is wrong (secrets required)' do
          ...
          expect do
            subject._connect('SecureNet',
              'wrongpass')
          end.to raise_error(WifiWand::NetworkAuthenticationError, /SecureNet/)
        end
        ...
        it 'raises NetworkConnectionError for generic activation failures (out of range)' do
          ...
          expect do
            subject._connect('WeakSignal')
          end.to raise_error(WifiWand::NetworkConnectionError, /out of range/)
        end
        ...
        it 're-raises unknown errors from nmcli' do
          ...
          expect do
            subject._connect('SSID6',
              'pw')
          end.to raise_error(WifiWand::CommandExecutor::OsCommandError, /Unknown system failure/)
        end
      end
      ...
      describe '#remove_preferred_network' do
        ...
        it 'deletes existing preferred network and returns nil' do
          ...
          expect(subject).to receive(:run_os_command).with(['nmcli', 'connection', 'delete',
                                                            'Home'])
      ...
      describe '#_disconnect' do
        it 'returns nil when disconnect succeeds' do
          ...
          expect(subject).to receive(:run_os_command).with(['nmcli', 'dev', 'disconnect',
      ...
      describe '#default_interface' do
        it 'returns nil when ip route command fails' do
          expect(subject).to receive(:run_os_command).with(['ip', 'route', 'show', 'default'],
            false)
