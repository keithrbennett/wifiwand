...
Use the cov-loupe MCP server to analyze the test coverage data generated by SimpleCov for this Ruby on Rails
project. Prefer MCP tools such as `project_coverage`, `file_coverage_summary`, and `file_uncovered_lines` over
reading resultsets directly; only fall back to the `cov-loupe` CLI if the MCP server is unavailable. Generate
a detailed coverage analysis report and output it to a markdown file named `test_coverage_analysis.md`.
...
Organize by:
- **Complexity to Test**: Simple/Moderate/Complex (considering Rails dependencies, database setup, external
  services, etc.)
