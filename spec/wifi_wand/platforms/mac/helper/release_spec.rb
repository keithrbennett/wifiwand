...
RSpec.describe WifiWand::Platforms::Mac::Helper::Release do
  ...
  describe '.fetch_notary_credentials!' do
    ...
    it 'uses the official maintainer Team ID when the environment is unset' do
      ...
      ).to include(team_id: WifiWand::Platforms::Mac::Helper::Bundle::OFFICIAL_APPLE_TEAM_ID)
    end
  end
  ...
  describe '.build_signed_helper' do
    ...
    let(:official_identity) { WifiWand::Platforms::Mac::Helper::Bundle::OFFICIAL_CODESIGN_IDENTITY }
  ...
  describe '.notarize_helper' do
    ...
    before do
      ...
      allow(described_class).to receive(:fetch_notary_credentials!).and_return(creds)
      allow(described_class).to receive(:verify_source_attestation!)
      allow(WifiWand::Platforms::Mac::Helper::Bundle).to receive(:source_bundle_path).and_return(bundle_path)
